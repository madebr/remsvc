...
public:
    ...
    inline void Update(const void *data, size_t size) {
        sig = SigForPbCb(data, size, m_value);
    }
private:
    uint32_t sig;
};
...
public:
    ...
    inline void FlushWrite() {
        if (m_write_end != m_write_left) {
            if (((WarnIsError > 1) + Nerrors) == 0 && !gOption_E && !Out_funcdef) {
                size_t written = 0;
                if (m_field_0x18) {
...
#include "alloc.h"
extern VirtualHeap g_PCH_allocated_chunk_004664a0;
...
public:
    tStruct_0045e478() {
        m_field_0x00 = SourceBrowserNam != NULL;
        m_field_0x08 = pSbr;
    }
private:
    BOOL m_field_0x00;
